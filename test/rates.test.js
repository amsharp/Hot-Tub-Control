...
const RATES = { summerOn: 0.5, summerOff: 0.3, winterMid: 0.4, winterSoff: 0.2, winterOff: 0.3 };
const s = new TouSchedule(RATES);
const at = (m, h) => new Date(2026, m - 1, 15, h, 30); // local time

test('summer: on-peak 4-9 PM, off-peak otherwise', () => {
  assert.equal(s.rateAt(at(7, 17)), 0.5); // July 5:30 PM
  assert.equal(s.rateAt(at(7, 15)), 0.3); // July 3:30 PM
